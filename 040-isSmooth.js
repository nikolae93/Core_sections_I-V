...
The first and the last elements of arr are equal to 7, and its middle also equals 2 + 5 = 7. Thus, 
the array is smooth and the output is true.

For arr = [-5, -5, 10], the output should be
isSmooth(arr) = false.

The first and middle elements are equal to -5, but the last element equals 10. Thus, arr is not smooth and
 the output is false.
...
function isSmooth(arr) {
    if(arr.length%2===0){
       let m = arr[arr.length/2]+arr[arr.length/2-1];
      
       if(arr[0]===arr[arr.length-1] && arr[arr.length-1]===m){return true;}
       else {return false;}
       ...
       if(arr[0]===arr[arr.length-1] && arr[arr.length-1]===arr[middle] ){return true;}
       else{return false;}
    }
 }